...
struct generatedFcnInterest {
			   ...
			   const Matrix<REAL_T> *I, Matrix<REAL_T> *pred)
  {
    ...
    o = p[1];
    //h1 += in[0] * p[2];
    h1 = DATA_TYPE::fma(h1,in[0],p[2]);
    //h1 += in[1] * p[3];
    h1 = DATA_TYPE::fma(h1,in[1],p[3]);
    h1 = DATA_TYPE::vecG(h1);
    //o += in[0] * p[4];
    o = DATA_TYPE::fma(o,in[0],p[4]);
    //o += in[1] * p[5];
    o = DATA_TYPE::fma(o,in[1],p[5]);
    //o += h1 * p[6];
    o = DATA_TYPE::fma(o,h1,p[6]);
    if(IS_PRED == true) {
      (*pred)(exampleNumber,0) = o;
      return 0.;
    } else {
      REAL_T d = DATA_TYPE::sub(o,(*pred)(exampleNumber,0));
      d = DATA_TYPE::mult(d, d);
      return DATA_TYPE::reduce(d);
    }
#endif
  }
