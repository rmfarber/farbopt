#ifndef PCA_HPP
#define PCA_HPP
#include "Matrix.hpp"
#ifdef USE_GRAD
#include <adolc/adolc.h>
#endif
#include "Gfcn.h"

#ifndef FCN_ATTRIBUTES
#define FCN_ATTRIBUTES ""
#endif

#define N_INPUT (16)
#define N_H1 (10)
#define N_H2 (1)
#define N_H3 (10)
#define N_OUTPUT (0)
#define EXAMPLE_SIZE (16)
#define N_PARAM (377)
#define FLOP_ESTIMATE (730 + 20 * G_ESTIMATE)
template<typename REAL_T>
struct generatedFcnInterest {
  FCN_ATTRIBUTES
  inline uint32_t nInput() {return N_INPUT;}
  FCN_ATTRIBUTES
  inline uint32_t nOutput() {return N_OUTPUT;}
  FCN_ATTRIBUTES
  inline uint32_t nParam() { return N_PARAM; }
  FCN_ATTRIBUTES
  inline uint32_t nFlop() {return FLOP_ESTIMATE;}
  FCN_ATTRIBUTES 
   inline const char* name() {return "autoencoder 16x10x1x10x16 function";}

  FCN_ATTRIBUTES
  inline const char* gFcnName() {return G_DESC_STRING; }
  
  template<bool IS_PRED>
  FCN_ATTRIBUTES
  inline float generic_fcn(const REAL_T *p, const REAL_T *I, REAL_T *pred)

{
   float in[16];
   in[0] = I[0];
   in[1] = I[1];
   in[2] = I[2];
   in[3] = I[3];
   in[4] = I[4];
   in[5] = I[5];
   in[6] = I[6];
   in[7] = I[7];
   in[8] = I[8];
   in[9] = I[9];
   in[10] = I[10];
   in[11] = I[11];
   in[12] = I[12];
   in[13] = I[13];
   in[14] = I[14];
   in[15] = I[15];
   register float h1_0 = p[0];
   register float h1_1 = p[1];
   register float h1_2 = p[2];
   register float h1_3 = p[3];
   register float h1_4 = p[4];
   register float h1_5 = p[5];
   register float h1_6 = p[6];
   register float h1_7 = p[7];
   register float h1_8 = p[8];
   register float h1_9 = p[9];
   h1_0 += in[0] * p[10];
   h1_1 += in[0] * p[11];
   h1_2 += in[0] * p[12];
   h1_3 += in[0] * p[13];
   h1_4 += in[0] * p[14];
   h1_5 += in[0] * p[15];
   h1_6 += in[0] * p[16];
   h1_7 += in[0] * p[17];
   h1_8 += in[0] * p[18];
   h1_9 += in[0] * p[19];
   h1_0 += in[1] * p[20];
   h1_1 += in[1] * p[21];
   h1_2 += in[1] * p[22];
   h1_3 += in[1] * p[23];
   h1_4 += in[1] * p[24];
   h1_5 += in[1] * p[25];
   h1_6 += in[1] * p[26];
   h1_7 += in[1] * p[27];
   h1_8 += in[1] * p[28];
   h1_9 += in[1] * p[29];
   h1_0 += in[2] * p[30];
   h1_1 += in[2] * p[31];
   h1_2 += in[2] * p[32];
   h1_3 += in[2] * p[33];
   h1_4 += in[2] * p[34];
   h1_5 += in[2] * p[35];
   h1_6 += in[2] * p[36];
   h1_7 += in[2] * p[37];
   h1_8 += in[2] * p[38];
   h1_9 += in[2] * p[39];
   h1_0 += in[3] * p[40];
   h1_1 += in[3] * p[41];
   h1_2 += in[3] * p[42];
   h1_3 += in[3] * p[43];
   h1_4 += in[3] * p[44];
   h1_5 += in[3] * p[45];
   h1_6 += in[3] * p[46];
   h1_7 += in[3] * p[47];
   h1_8 += in[3] * p[48];
   h1_9 += in[3] * p[49];
   h1_0 += in[4] * p[50];
   h1_1 += in[4] * p[51];
   h1_2 += in[4] * p[52];
   h1_3 += in[4] * p[53];
   h1_4 += in[4] * p[54];
   h1_5 += in[4] * p[55];
   h1_6 += in[4] * p[56];
   h1_7 += in[4] * p[57];
   h1_8 += in[4] * p[58];
   h1_9 += in[4] * p[59];
   h1_0 += in[5] * p[60];
   h1_1 += in[5] * p[61];
   h1_2 += in[5] * p[62];
   h1_3 += in[5] * p[63];
   h1_4 += in[5] * p[64];
   h1_5 += in[5] * p[65];
   h1_6 += in[5] * p[66];
   h1_7 += in[5] * p[67];
   h1_8 += in[5] * p[68];
   h1_9 += in[5] * p[69];
   h1_0 += in[6] * p[70];
   h1_1 += in[6] * p[71];
   h1_2 += in[6] * p[72];
   h1_3 += in[6] * p[73];
   h1_4 += in[6] * p[74];
   h1_5 += in[6] * p[75];
   h1_6 += in[6] * p[76];
   h1_7 += in[6] * p[77];
   h1_8 += in[6] * p[78];
   h1_9 += in[6] * p[79];
   h1_0 += in[7] * p[80];
   h1_1 += in[7] * p[81];
   h1_2 += in[7] * p[82];
   h1_3 += in[7] * p[83];
   h1_4 += in[7] * p[84];
   h1_5 += in[7] * p[85];
   h1_6 += in[7] * p[86];
   h1_7 += in[7] * p[87];
   h1_8 += in[7] * p[88];
   h1_9 += in[7] * p[89];
   h1_0 += in[8] * p[90];
   h1_1 += in[8] * p[91];
   h1_2 += in[8] * p[92];
   h1_3 += in[8] * p[93];
   h1_4 += in[8] * p[94];
   h1_5 += in[8] * p[95];
   h1_6 += in[8] * p[96];
   h1_7 += in[8] * p[97];
   h1_8 += in[8] * p[98];
   h1_9 += in[8] * p[99];
   h1_0 += in[9] * p[100];
   h1_1 += in[9] * p[101];
   h1_2 += in[9] * p[102];
   h1_3 += in[9] * p[103];
   h1_4 += in[9] * p[104];
   h1_5 += in[9] * p[105];
   h1_6 += in[9] * p[106];
   h1_7 += in[9] * p[107];
   h1_8 += in[9] * p[108];
   h1_9 += in[9] * p[109];
   h1_0 += in[10] * p[110];
   h1_1 += in[10] * p[111];
   h1_2 += in[10] * p[112];
   h1_3 += in[10] * p[113];
   h1_4 += in[10] * p[114];
   h1_5 += in[10] * p[115];
   h1_6 += in[10] * p[116];
   h1_7 += in[10] * p[117];
   h1_8 += in[10] * p[118];
   h1_9 += in[10] * p[119];
   h1_0 += in[11] * p[120];
   h1_1 += in[11] * p[121];
   h1_2 += in[11] * p[122];
   h1_3 += in[11] * p[123];
   h1_4 += in[11] * p[124];
   h1_5 += in[11] * p[125];
   h1_6 += in[11] * p[126];
   h1_7 += in[11] * p[127];
   h1_8 += in[11] * p[128];
   h1_9 += in[11] * p[129];
   h1_0 += in[12] * p[130];
   h1_1 += in[12] * p[131];
   h1_2 += in[12] * p[132];
   h1_3 += in[12] * p[133];
   h1_4 += in[12] * p[134];
   h1_5 += in[12] * p[135];
   h1_6 += in[12] * p[136];
   h1_7 += in[12] * p[137];
   h1_8 += in[12] * p[138];
   h1_9 += in[12] * p[139];
   h1_0 += in[13] * p[140];
   h1_1 += in[13] * p[141];
   h1_2 += in[13] * p[142];
   h1_3 += in[13] * p[143];
   h1_4 += in[13] * p[144];
   h1_5 += in[13] * p[145];
   h1_6 += in[13] * p[146];
   h1_7 += in[13] * p[147];
   h1_8 += in[13] * p[148];
   h1_9 += in[13] * p[149];
   h1_0 += in[14] * p[150];
   h1_1 += in[14] * p[151];
   h1_2 += in[14] * p[152];
   h1_3 += in[14] * p[153];
   h1_4 += in[14] * p[154];
   h1_5 += in[14] * p[155];
   h1_6 += in[14] * p[156];
   h1_7 += in[14] * p[157];
   h1_8 += in[14] * p[158];
   h1_9 += in[14] * p[159];
   h1_0 += in[15] * p[160];
   h1_1 += in[15] * p[161];
   h1_2 += in[15] * p[162];
   h1_3 += in[15] * p[163];
   h1_4 += in[15] * p[164];
   h1_5 += in[15] * p[165];
   h1_6 += in[15] * p[166];
   h1_7 += in[15] * p[167];
   h1_8 += in[15] * p[168];
   h1_9 += in[15] * p[169];
   h1_0 = G(h1_0);
   h1_1 = G(h1_1);
   h1_2 = G(h1_2);
   h1_3 = G(h1_3);
   h1_4 = G(h1_4);
   h1_5 = G(h1_5);
   h1_6 = G(h1_6);
   h1_7 = G(h1_7);
   h1_8 = G(h1_8);
   h1_9 = G(h1_9);
   register float h2_0 = p[170];
   h2_0 += h1_0 * p[171];
   h2_0 += h1_1 * p[172];
   h2_0 += h1_2 * p[173];
   h2_0 += h1_3 * p[174];
   h2_0 += h1_4 * p[175];
   h2_0 += h1_5 * p[176];
   h2_0 += h1_6 * p[177];
   h2_0 += h1_7 * p[178];
   h2_0 += h1_8 * p[179];
   h2_0 += h1_9 * p[180];
   register float h3_0 = p[181];
   register float h3_1 = p[182];
   register float h3_2 = p[183];
   register float h3_3 = p[184];
   register float h3_4 = p[185];
   register float h3_5 = p[186];
   register float h3_6 = p[187];
   register float h3_7 = p[188];
   register float h3_8 = p[189];
   register float h3_9 = p[190];
   h3_0 += h2_0 * p[191];
   h3_1 += h2_0 * p[192];
   h3_2 += h2_0 * p[193];
   h3_3 += h2_0 * p[194];
   h3_4 += h2_0 * p[195];
   h3_5 += h2_0 * p[196];
   h3_6 += h2_0 * p[197];
   h3_7 += h2_0 * p[198];
   h3_8 += h2_0 * p[199];
   h3_9 += h2_0 * p[200];
   h3_0 = G(h3_0);
   h3_1 = G(h3_1);
   h3_2 = G(h3_2);
   h3_3 = G(h3_3);
   h3_4 = G(h3_4);
   h3_5 = G(h3_5);
   h3_6 = G(h3_6);
   h3_7 = G(h3_7);
   h3_8 = G(h3_8);
   h3_9 = G(h3_9);
   register float o,sum = 0.f;
   o = p[201];
   o += h3_0 * p[202];
   o += h3_1 * p[203];
   o += h3_2 * p[204];
   o += h3_3 * p[205];
   o += h3_4 * p[206];
   o += h3_5 * p[207];
   o += h3_6 * p[208];
   o += h3_7 * p[209];
   o += h3_8 * p[210];
   o += h3_9 * p[211];
   if(IS_PRED == true) {
      pred[0] = o;
   }
   o -= in[0];
   sum += o*o;
   o = p[212];
   o += h3_0 * p[213];
   o += h3_1 * p[214];
   o += h3_2 * p[215];
   o += h3_3 * p[216];
   o += h3_4 * p[217];
   o += h3_5 * p[218];
   o += h3_6 * p[219];
   o += h3_7 * p[220];
   o += h3_8 * p[221];
   o += h3_9 * p[222];
   if(IS_PRED == true) {
      pred[1] = o;
   }
   o -= in[1];
   sum += o*o;
   o = p[223];
   o += h3_0 * p[224];
   o += h3_1 * p[225];
   o += h3_2 * p[226];
   o += h3_3 * p[227];
   o += h3_4 * p[228];
   o += h3_5 * p[229];
   o += h3_6 * p[230];
   o += h3_7 * p[231];
   o += h3_8 * p[232];
   o += h3_9 * p[233];
   if(IS_PRED == true) {
      pred[2] = o;
   }
   o -= in[2];
   sum += o*o;
   o = p[234];
   o += h3_0 * p[235];
   o += h3_1 * p[236];
   o += h3_2 * p[237];
   o += h3_3 * p[238];
   o += h3_4 * p[239];
   o += h3_5 * p[240];
   o += h3_6 * p[241];
   o += h3_7 * p[242];
   o += h3_8 * p[243];
   o += h3_9 * p[244];
   if(IS_PRED == true) {
      pred[3] = o;
   }
   o -= in[3];
   sum += o*o;
   o = p[245];
   o += h3_0 * p[246];
   o += h3_1 * p[247];
   o += h3_2 * p[248];
   o += h3_3 * p[249];
   o += h3_4 * p[250];
   o += h3_5 * p[251];
   o += h3_6 * p[252];
   o += h3_7 * p[253];
   o += h3_8 * p[254];
   o += h3_9 * p[255];
   if(IS_PRED == true) {
      pred[4] = o;
   }
   o -= in[4];
   sum += o*o;
   o = p[256];
   o += h3_0 * p[257];
   o += h3_1 * p[258];
   o += h3_2 * p[259];
   o += h3_3 * p[260];
   o += h3_4 * p[261];
   o += h3_5 * p[262];
   o += h3_6 * p[263];
   o += h3_7 * p[264];
   o += h3_8 * p[265];
   o += h3_9 * p[266];
   if(IS_PRED == true) {
      pred[5] = o;
   }
   o -= in[5];
   sum += o*o;
   o = p[267];
   o += h3_0 * p[268];
   o += h3_1 * p[269];
   o += h3_2 * p[270];
   o += h3_3 * p[271];
   o += h3_4 * p[272];
   o += h3_5 * p[273];
   o += h3_6 * p[274];
   o += h3_7 * p[275];
   o += h3_8 * p[276];
   o += h3_9 * p[277];
   if(IS_PRED == true) {
      pred[6] = o;
   }
   o -= in[6];
   sum += o*o;
   o = p[278];
   o += h3_0 * p[279];
   o += h3_1 * p[280];
   o += h3_2 * p[281];
   o += h3_3 * p[282];
   o += h3_4 * p[283];
   o += h3_5 * p[284];
   o += h3_6 * p[285];
   o += h3_7 * p[286];
   o += h3_8 * p[287];
   o += h3_9 * p[288];
   if(IS_PRED == true) {
      pred[7] = o;
   }
   o -= in[7];
   sum += o*o;
   o = p[289];
   o += h3_0 * p[290];
   o += h3_1 * p[291];
   o += h3_2 * p[292];
   o += h3_3 * p[293];
   o += h3_4 * p[294];
   o += h3_5 * p[295];
   o += h3_6 * p[296];
   o += h3_7 * p[297];
   o += h3_8 * p[298];
   o += h3_9 * p[299];
   if(IS_PRED == true) {
      pred[8] = o;
   }
   o -= in[8];
   sum += o*o;
   o = p[300];
   o += h3_0 * p[301];
   o += h3_1 * p[302];
   o += h3_2 * p[303];
   o += h3_3 * p[304];
   o += h3_4 * p[305];
   o += h3_5 * p[306];
   o += h3_6 * p[307];
   o += h3_7 * p[308];
   o += h3_8 * p[309];
   o += h3_9 * p[310];
   if(IS_PRED == true) {
      pred[9] = o;
   }
   o -= in[9];
   sum += o*o;
   o = p[311];
   o += h3_0 * p[312];
   o += h3_1 * p[313];
   o += h3_2 * p[314];
   o += h3_3 * p[315];
   o += h3_4 * p[316];
   o += h3_5 * p[317];
   o += h3_6 * p[318];
   o += h3_7 * p[319];
   o += h3_8 * p[320];
   o += h3_9 * p[321];
   if(IS_PRED == true) {
      pred[10] = o;
   }
   o -= in[10];
   sum += o*o;
   o = p[322];
   o += h3_0 * p[323];
   o += h3_1 * p[324];
   o += h3_2 * p[325];
   o += h3_3 * p[326];
   o += h3_4 * p[327];
   o += h3_5 * p[328];
   o += h3_6 * p[329];
   o += h3_7 * p[330];
   o += h3_8 * p[331];
   o += h3_9 * p[332];
   if(IS_PRED == true) {
      pred[11] = o;
   }
   o -= in[11];
   sum += o*o;
   o = p[333];
   o += h3_0 * p[334];
   o += h3_1 * p[335];
   o += h3_2 * p[336];
   o += h3_3 * p[337];
   o += h3_4 * p[338];
   o += h3_5 * p[339];
   o += h3_6 * p[340];
   o += h3_7 * p[341];
   o += h3_8 * p[342];
   o += h3_9 * p[343];
   if(IS_PRED == true) {
      pred[12] = o;
   }
   o -= in[12];
   sum += o*o;
   o = p[344];
   o += h3_0 * p[345];
   o += h3_1 * p[346];
   o += h3_2 * p[347];
   o += h3_3 * p[348];
   o += h3_4 * p[349];
   o += h3_5 * p[350];
   o += h3_6 * p[351];
   o += h3_7 * p[352];
   o += h3_8 * p[353];
   o += h3_9 * p[354];
   if(IS_PRED == true) {
      pred[13] = o;
   }
   o -= in[13];
   sum += o*o;
   o = p[355];
   o += h3_0 * p[356];
   o += h3_1 * p[357];
   o += h3_2 * p[358];
   o += h3_3 * p[359];
   o += h3_4 * p[360];
   o += h3_5 * p[361];
   o += h3_6 * p[362];
   o += h3_7 * p[363];
   o += h3_8 * p[364];
   o += h3_9 * p[365];
   if(IS_PRED == true) {
      pred[14] = o;
   }
   o -= in[14];
   sum += o*o;
   o = p[366];
   o += h3_0 * p[367];
   o += h3_1 * p[368];
   o += h3_2 * p[369];
   o += h3_3 * p[370];
   o += h3_4 * p[371];
   o += h3_5 * p[372];
   o += h3_6 * p[373];
   o += h3_7 * p[374];
   o += h3_8 * p[375];
   o += h3_9 * p[376];
   if(IS_PRED == true) {
      pred[15] = o;
      return 0.;
   }
   o -= in[15];
   sum += o*o;
   return(sum);
}


  adouble ad_fcn(const uint32_t exampleNumber, const adouble *p,
                           const Matrix<REAL_T> *I, Matrix<REAL_T> *pred)

{
   adouble in[16];
   in[0] = mkparam( (*I)(exampleNumber,0) );
   in[1] = mkparam( (*I)(exampleNumber,1) );
   in[2] = mkparam( (*I)(exampleNumber,2) );
   in[3] = mkparam( (*I)(exampleNumber,3) );
   in[4] = mkparam( (*I)(exampleNumber,4) );
   in[5] = mkparam( (*I)(exampleNumber,5) );
   in[6] = mkparam( (*I)(exampleNumber,6) );
   in[7] = mkparam( (*I)(exampleNumber,7) );
   in[8] = mkparam( (*I)(exampleNumber,8) );
   in[9] = mkparam( (*I)(exampleNumber,9) );
   in[10] = mkparam( (*I)(exampleNumber,10) );
   in[11] = mkparam( (*I)(exampleNumber,11) );
   in[12] = mkparam( (*I)(exampleNumber,12) );
   in[13] = mkparam( (*I)(exampleNumber,13) );
   in[14] = mkparam( (*I)(exampleNumber,14) );
   in[15] = mkparam( (*I)(exampleNumber,15) );
   adouble h1_0 = p[0];
   adouble h1_1 = p[1];
   adouble h1_2 = p[2];
   adouble h1_3 = p[3];
   adouble h1_4 = p[4];
   adouble h1_5 = p[5];
   adouble h1_6 = p[6];
   adouble h1_7 = p[7];
   adouble h1_8 = p[8];
   adouble h1_9 = p[9];
   h1_0 += in[0] * p[10];
   h1_1 += in[0] * p[11];
   h1_2 += in[0] * p[12];
   h1_3 += in[0] * p[13];
   h1_4 += in[0] * p[14];
   h1_5 += in[0] * p[15];
   h1_6 += in[0] * p[16];
   h1_7 += in[0] * p[17];
   h1_8 += in[0] * p[18];
   h1_9 += in[0] * p[19];
   h1_0 += in[1] * p[20];
   h1_1 += in[1] * p[21];
   h1_2 += in[1] * p[22];
   h1_3 += in[1] * p[23];
   h1_4 += in[1] * p[24];
   h1_5 += in[1] * p[25];
   h1_6 += in[1] * p[26];
   h1_7 += in[1] * p[27];
   h1_8 += in[1] * p[28];
   h1_9 += in[1] * p[29];
   h1_0 += in[2] * p[30];
   h1_1 += in[2] * p[31];
   h1_2 += in[2] * p[32];
   h1_3 += in[2] * p[33];
   h1_4 += in[2] * p[34];
   h1_5 += in[2] * p[35];
   h1_6 += in[2] * p[36];
   h1_7 += in[2] * p[37];
   h1_8 += in[2] * p[38];
   h1_9 += in[2] * p[39];
   h1_0 += in[3] * p[40];
   h1_1 += in[3] * p[41];
   h1_2 += in[3] * p[42];
   h1_3 += in[3] * p[43];
   h1_4 += in[3] * p[44];
   h1_5 += in[3] * p[45];
   h1_6 += in[3] * p[46];
   h1_7 += in[3] * p[47];
   h1_8 += in[3] * p[48];
   h1_9 += in[3] * p[49];
   h1_0 += in[4] * p[50];
   h1_1 += in[4] * p[51];
   h1_2 += in[4] * p[52];
   h1_3 += in[4] * p[53];
   h1_4 += in[4] * p[54];
   h1_5 += in[4] * p[55];
   h1_6 += in[4] * p[56];
   h1_7 += in[4] * p[57];
   h1_8 += in[4] * p[58];
   h1_9 += in[4] * p[59];
   h1_0 += in[5] * p[60];
   h1_1 += in[5] * p[61];
   h1_2 += in[5] * p[62];
   h1_3 += in[5] * p[63];
   h1_4 += in[5] * p[64];
   h1_5 += in[5] * p[65];
   h1_6 += in[5] * p[66];
   h1_7 += in[5] * p[67];
   h1_8 += in[5] * p[68];
   h1_9 += in[5] * p[69];
   h1_0 += in[6] * p[70];
   h1_1 += in[6] * p[71];
   h1_2 += in[6] * p[72];
   h1_3 += in[6] * p[73];
   h1_4 += in[6] * p[74];
   h1_5 += in[6] * p[75];
   h1_6 += in[6] * p[76];
   h1_7 += in[6] * p[77];
   h1_8 += in[6] * p[78];
   h1_9 += in[6] * p[79];
   h1_0 += in[7] * p[80];
   h1_1 += in[7] * p[81];
   h1_2 += in[7] * p[82];
   h1_3 += in[7] * p[83];
   h1_4 += in[7] * p[84];
   h1_5 += in[7] * p[85];
   h1_6 += in[7] * p[86];
   h1_7 += in[7] * p[87];
   h1_8 += in[7] * p[88];
   h1_9 += in[7] * p[89];
   h1_0 += in[8] * p[90];
   h1_1 += in[8] * p[91];
   h1_2 += in[8] * p[92];
   h1_3 += in[8] * p[93];
   h1_4 += in[8] * p[94];
   h1_5 += in[8] * p[95];
   h1_6 += in[8] * p[96];
   h1_7 += in[8] * p[97];
   h1_8 += in[8] * p[98];
   h1_9 += in[8] * p[99];
   h1_0 += in[9] * p[100];
   h1_1 += in[9] * p[101];
   h1_2 += in[9] * p[102];
   h1_3 += in[9] * p[103];
   h1_4 += in[9] * p[104];
   h1_5 += in[9] * p[105];
   h1_6 += in[9] * p[106];
   h1_7 += in[9] * p[107];
   h1_8 += in[9] * p[108];
   h1_9 += in[9] * p[109];
   h1_0 += in[10] * p[110];
   h1_1 += in[10] * p[111];
   h1_2 += in[10] * p[112];
   h1_3 += in[10] * p[113];
   h1_4 += in[10] * p[114];
   h1_5 += in[10] * p[115];
   h1_6 += in[10] * p[116];
   h1_7 += in[10] * p[117];
   h1_8 += in[10] * p[118];
   h1_9 += in[10] * p[119];
   h1_0 += in[11] * p[120];
   h1_1 += in[11] * p[121];
   h1_2 += in[11] * p[122];
   h1_3 += in[11] * p[123];
   h1_4 += in[11] * p[124];
   h1_5 += in[11] * p[125];
   h1_6 += in[11] * p[126];
   h1_7 += in[11] * p[127];
   h1_8 += in[11] * p[128];
   h1_9 += in[11] * p[129];
   h1_0 += in[12] * p[130];
   h1_1 += in[12] * p[131];
   h1_2 += in[12] * p[132];
   h1_3 += in[12] * p[133];
   h1_4 += in[12] * p[134];
   h1_5 += in[12] * p[135];
   h1_6 += in[12] * p[136];
   h1_7 += in[12] * p[137];
   h1_8 += in[12] * p[138];
   h1_9 += in[12] * p[139];
   h1_0 += in[13] * p[140];
   h1_1 += in[13] * p[141];
   h1_2 += in[13] * p[142];
   h1_3 += in[13] * p[143];
   h1_4 += in[13] * p[144];
   h1_5 += in[13] * p[145];
   h1_6 += in[13] * p[146];
   h1_7 += in[13] * p[147];
   h1_8 += in[13] * p[148];
   h1_9 += in[13] * p[149];
   h1_0 += in[14] * p[150];
   h1_1 += in[14] * p[151];
   h1_2 += in[14] * p[152];
   h1_3 += in[14] * p[153];
   h1_4 += in[14] * p[154];
   h1_5 += in[14] * p[155];
   h1_6 += in[14] * p[156];
   h1_7 += in[14] * p[157];
   h1_8 += in[14] * p[158];
   h1_9 += in[14] * p[159];
   h1_0 += in[15] * p[160];
   h1_1 += in[15] * p[161];
   h1_2 += in[15] * p[162];
   h1_3 += in[15] * p[163];
   h1_4 += in[15] * p[164];
   h1_5 += in[15] * p[165];
   h1_6 += in[15] * p[166];
   h1_7 += in[15] * p[167];
   h1_8 += in[15] * p[168];
   h1_9 += in[15] * p[169];
   h1_0 = G_ad(h1_0);
   h1_1 = G_ad(h1_1);
   h1_2 = G_ad(h1_2);
   h1_3 = G_ad(h1_3);
   h1_4 = G_ad(h1_4);
   h1_5 = G_ad(h1_5);
   h1_6 = G_ad(h1_6);
   h1_7 = G_ad(h1_7);
   h1_8 = G_ad(h1_8);
   h1_9 = G_ad(h1_9);
   adouble h2_0 = p[170];
   h2_0 += h1_0 * p[171];
   h2_0 += h1_1 * p[172];
   h2_0 += h1_2 * p[173];
   h2_0 += h1_3 * p[174];
   h2_0 += h1_4 * p[175];
   h2_0 += h1_5 * p[176];
   h2_0 += h1_6 * p[177];
   h2_0 += h1_7 * p[178];
   h2_0 += h1_8 * p[179];
   h2_0 += h1_9 * p[180];
   adouble h3_0 = p[181];
   adouble h3_1 = p[182];
   adouble h3_2 = p[183];
   adouble h3_3 = p[184];
   adouble h3_4 = p[185];
   adouble h3_5 = p[186];
   adouble h3_6 = p[187];
   adouble h3_7 = p[188];
   adouble h3_8 = p[189];
   adouble h3_9 = p[190];
   h3_0 += h2_0 * p[191];
   h3_1 += h2_0 * p[192];
   h3_2 += h2_0 * p[193];
   h3_3 += h2_0 * p[194];
   h3_4 += h2_0 * p[195];
   h3_5 += h2_0 * p[196];
   h3_6 += h2_0 * p[197];
   h3_7 += h2_0 * p[198];
   h3_8 += h2_0 * p[199];
   h3_9 += h2_0 * p[200];
   h3_0 = G_ad(h3_0);
   h3_1 = G_ad(h3_1);
   h3_2 = G_ad(h3_2);
   h3_3 = G_ad(h3_3);
   h3_4 = G_ad(h3_4);
   h3_5 = G_ad(h3_5);
   h3_6 = G_ad(h3_6);
   h3_7 = G_ad(h3_7);
   h3_8 = G_ad(h3_8);
   h3_9 = G_ad(h3_9);
   adouble o,sum = 0.f;
   o = p[201];
   o += h3_0 * p[202];
   o += h3_1 * p[203];
   o += h3_2 * p[204];
   o += h3_3 * p[205];
   o += h3_4 * p[206];
   o += h3_5 * p[207];
   o += h3_6 * p[208];
   o += h3_7 * p[209];
   o += h3_8 * p[210];
   o += h3_9 * p[211];
   o -= in[0];
   sum += o*o;
   o = p[212];
   o += h3_0 * p[213];
   o += h3_1 * p[214];
   o += h3_2 * p[215];
   o += h3_3 * p[216];
   o += h3_4 * p[217];
   o += h3_5 * p[218];
   o += h3_6 * p[219];
   o += h3_7 * p[220];
   o += h3_8 * p[221];
   o += h3_9 * p[222];
   o -= in[1];
   sum += o*o;
   o = p[223];
   o += h3_0 * p[224];
   o += h3_1 * p[225];
   o += h3_2 * p[226];
   o += h3_3 * p[227];
   o += h3_4 * p[228];
   o += h3_5 * p[229];
   o += h3_6 * p[230];
   o += h3_7 * p[231];
   o += h3_8 * p[232];
   o += h3_9 * p[233];
   o -= in[2];
   sum += o*o;
   o = p[234];
   o += h3_0 * p[235];
   o += h3_1 * p[236];
   o += h3_2 * p[237];
   o += h3_3 * p[238];
   o += h3_4 * p[239];
   o += h3_5 * p[240];
   o += h3_6 * p[241];
   o += h3_7 * p[242];
   o += h3_8 * p[243];
   o += h3_9 * p[244];
   o -= in[3];
   sum += o*o;
   o = p[245];
   o += h3_0 * p[246];
   o += h3_1 * p[247];
   o += h3_2 * p[248];
   o += h3_3 * p[249];
   o += h3_4 * p[250];
   o += h3_5 * p[251];
   o += h3_6 * p[252];
   o += h3_7 * p[253];
   o += h3_8 * p[254];
   o += h3_9 * p[255];
   o -= in[4];
   sum += o*o;
   o = p[256];
   o += h3_0 * p[257];
   o += h3_1 * p[258];
   o += h3_2 * p[259];
   o += h3_3 * p[260];
   o += h3_4 * p[261];
   o += h3_5 * p[262];
   o += h3_6 * p[263];
   o += h3_7 * p[264];
   o += h3_8 * p[265];
   o += h3_9 * p[266];
   o -= in[5];
   sum += o*o;
   o = p[267];
   o += h3_0 * p[268];
   o += h3_1 * p[269];
   o += h3_2 * p[270];
   o += h3_3 * p[271];
   o += h3_4 * p[272];
   o += h3_5 * p[273];
   o += h3_6 * p[274];
   o += h3_7 * p[275];
   o += h3_8 * p[276];
   o += h3_9 * p[277];
   o -= in[6];
   sum += o*o;
   o = p[278];
   o += h3_0 * p[279];
   o += h3_1 * p[280];
   o += h3_2 * p[281];
   o += h3_3 * p[282];
   o += h3_4 * p[283];
   o += h3_5 * p[284];
   o += h3_6 * p[285];
   o += h3_7 * p[286];
   o += h3_8 * p[287];
   o += h3_9 * p[288];
   o -= in[7];
   sum += o*o;
   o = p[289];
   o += h3_0 * p[290];
   o += h3_1 * p[291];
   o += h3_2 * p[292];
   o += h3_3 * p[293];
   o += h3_4 * p[294];
   o += h3_5 * p[295];
   o += h3_6 * p[296];
   o += h3_7 * p[297];
   o += h3_8 * p[298];
   o += h3_9 * p[299];
   o -= in[8];
   sum += o*o;
   o = p[300];
   o += h3_0 * p[301];
   o += h3_1 * p[302];
   o += h3_2 * p[303];
   o += h3_3 * p[304];
   o += h3_4 * p[305];
   o += h3_5 * p[306];
   o += h3_6 * p[307];
   o += h3_7 * p[308];
   o += h3_8 * p[309];
   o += h3_9 * p[310];
   o -= in[9];
   sum += o*o;
   o = p[311];
   o += h3_0 * p[312];
   o += h3_1 * p[313];
   o += h3_2 * p[314];
   o += h3_3 * p[315];
   o += h3_4 * p[316];
   o += h3_5 * p[317];
   o += h3_6 * p[318];
   o += h3_7 * p[319];
   o += h3_8 * p[320];
   o += h3_9 * p[321];
   o -= in[10];
   sum += o*o;
   o = p[322];
   o += h3_0 * p[323];
   o += h3_1 * p[324];
   o += h3_2 * p[325];
   o += h3_3 * p[326];
   o += h3_4 * p[327];
   o += h3_5 * p[328];
   o += h3_6 * p[329];
   o += h3_7 * p[330];
   o += h3_8 * p[331];
   o += h3_9 * p[332];
   o -= in[11];
   sum += o*o;
   o = p[333];
   o += h3_0 * p[334];
   o += h3_1 * p[335];
   o += h3_2 * p[336];
   o += h3_3 * p[337];
   o += h3_4 * p[338];
   o += h3_5 * p[339];
   o += h3_6 * p[340];
   o += h3_7 * p[341];
   o += h3_8 * p[342];
   o += h3_9 * p[343];
   o -= in[12];
   sum += o*o;
   o = p[344];
   o += h3_0 * p[345];
   o += h3_1 * p[346];
   o += h3_2 * p[347];
   o += h3_3 * p[348];
   o += h3_4 * p[349];
   o += h3_5 * p[350];
   o += h3_6 * p[351];
   o += h3_7 * p[352];
   o += h3_8 * p[353];
   o += h3_9 * p[354];
   o -= in[13];
   sum += o*o;
   o = p[355];
   o += h3_0 * p[356];
   o += h3_1 * p[357];
   o += h3_2 * p[358];
   o += h3_3 * p[359];
   o += h3_4 * p[360];
   o += h3_5 * p[361];
   o += h3_6 * p[362];
   o += h3_7 * p[363];
   o += h3_8 * p[364];
   o += h3_9 * p[365];
   o -= in[14];
   sum += o*o;
   o = p[366];
   o += h3_0 * p[367];
   o += h3_1 * p[368];
   o += h3_2 * p[369];
   o += h3_3 * p[370];
   o += h3_4 * p[371];
   o += h3_5 * p[372];
   o += h3_6 * p[373];
   o += h3_7 * p[374];
   o += h3_8 * p[375];
   o += h3_9 * p[376];
   o -= in[15];
   sum += o*o;
   return(sum);
}


  FCN_ATTRIBUTES
  inline void CalcOutput(const float *p, const REAL_T *I, REAL_T *pred)
  {
    generic_fcn<true>(p, I, pred);
  }
  
#pragma omp declare simd
  FCN_ATTRIBUTES
  inline float CalcOpt(const float *p, const REAL_T *I, const REAL_T *Known)
  {
    return generic_fcn<false>(p, I, const_cast< REAL_T *>(Known));
  }
};
#endif


